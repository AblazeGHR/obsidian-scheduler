...
// src/views/inline/inline-post-processor.ts
var INLINE_FIELD_RE2 = /\[([^\]:]+)::\s*([^\]]*)\]/g;
function createInlineCollapseProcessor(app, mapping) {
  return (el, ctx) => {
    const filePath = ctx.sourcePath;
    const items = el.querySelectorAll("li[data-line]");
    for (const item of items) {
      ...
      const fields = {};
      const rawMatches = [...html.matchAll(new RegExp(INLINE_FIELD_RE2.source, "g"))];
      for (const m3 of rawMatches) {
        fields[m3[1].trim()] = m3[2].trim();
      }
      const keys = Object.keys(fields);
      if (keys.length <= 3)
        continue;
      const visibleHtml = keys.slice(0, 3).map((k3) => ` <span class="inl-fld"><span class="inl-fld-key">${escapeHtml(k3)}</span>:<span class="inl-fld-val">${escapeHtml(fields[k3])}</span></span>`).join("");
      const hiddenFields = {};
      for (let i4 = 3; i4 < keys.length; i4++) {
        hiddenFields[keys[i4]] = fields[keys[i4]];
      }
      let cleanHtml = html.replace(new RegExp(INLINE_FIELD_RE2.source, "g"), "");
      ...
      const moreBtn = item.querySelector(`#${containerId}`);
      if (moreBtn) {
        moreBtn.addEventListener("click", (e3) => {
          e3.stopPropagation();
...
function openFieldsModal(app, mapping, fields, filePath, line, title, anchor) {
  ...
  function handleEdit(p3, field, value) {
